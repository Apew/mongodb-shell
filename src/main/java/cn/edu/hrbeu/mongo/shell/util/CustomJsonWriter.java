 ...
 * @since 3.0
 */
public class CustomJsonWriter extends AbstractBsonWriter {
     ...
     */
    public CustomJsonWriter(final Writer writer) {
     ...
     */
    public CustomJsonWriter(final Writer writer, final JsonWriterSettings settings) {
        super(settings);
...
//            }
    ...
    public void doWriteObjectId(final ObjectId objectId) {
        try {
            writeNameHelper(getName());
            writer.write(format("\"%s\"", objectId.toString()));
//            switch (settings.getOutputMode()) {
//                case STRICT:
//                    writeStartDocument();
